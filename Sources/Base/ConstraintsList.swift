...
extension UIView {
	...
	@objcMembers @objc(BunqConstraintsList) class ConstraintsList: NSObject {
		...
		var firstBaseline: NSLayoutConstraint?
		var lastBaseline: NSLayoutConstraint?

		var others = [NSLayoutConstraint]()
		...
		@discardableResult
		static func activate(_ constraints: [NSLayoutConstraint?]) -> ConstraintsList {
		...
		@discardableResult
		func changePriority(_ priority: UILayoutPriority) -> ConstraintsList {
		...
		func replaceConstraints(_ constraints: [NSLayoutConstraint]) {
			leading = nil
			trailing = nil
			top = nil
			bottom = nil
		...
		// replaces the existing constraints
		func replace(with block: @autoclosure () -> ConstraintsList) {
			deactivate()
			replaceConstraints(block().all)
		}

		func replace(by block: () -> ConstraintsList) {
			deactivate()
			replaceConstraints(block().all)
		}
